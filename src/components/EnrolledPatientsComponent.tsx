...
interface EnrolledPatientsComponentProps {
  selectedClinic: Clinic | null
}

export default function EnrolledPatientsComponent({ selectedClinic }: EnrolledPatientsComponentProps) {
  ...
  useEffect(() => {
    console.log("EnrolledPatientsComponent - selectedClinic:", selectedClinic);
    if (selectedClinic) {
      fetchPatients()
    }
  }, [selectedClinic])
  ...
  const fetchPatients = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4353/api/patients/enrolled?clinicId=${selectedClinic?.id}`)
